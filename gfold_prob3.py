...
# Just silence it
# Seems that cvxpy is comfused by the parameters
tst.no_warnings()
...
for k in range(0, prm.N):
    ...
    # μ1 * ( 1 - ( z(t) - z0(t) ) + ( z(t) - z0(t) ) ^ 2 / 2 ] <= s(t)

    # lower_bound = cp.SOC(
    #     0, 
    #     prm.mu_1[0, k] * ( 1 - var.z[0, k] + prm.z_0[0, k] + (var.z[0, k] - prm.z_0[0, k]) ** 2 / 2 ) - var.s[0, k]
    # )
    # tst.con_info(lower_bound)
    # # 1/2 \[Mu] (z^2-2 z z0-2 z+z0^2+2 z0+2)<=s
    # tst.expr_info(
    #     prm.mu_1[0, k]
    #     * (
    #         cp.square(var.z[0, k]) 
    #        - 2*var.z[0, k]*prm.z_0[0,k]
    #        - 2*var.z[0, k] 
    #        + cp.square(prm.z_0[0, k])
    #        + 2*prm.z_0[0, k]
    #        + 2
    #     ) / 2 - var.s[0, k])
    ...
    # tst.con_info(lower_bound)
    # # Print to file
    # with open("prob3_test_lb.txt", "a") as f:
    #     f.write(str(lower_bound) + "\n")
    #     f.write("DCP: " + str(lower_bound.is_dcp()) + "\n")
    #     f.write("DPP: " + str(lower_bound.is_dpp()) + "\n")
    #     f.write("\n")
